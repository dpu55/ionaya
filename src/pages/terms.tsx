...
const Terms: React.FC = () => {
  ...
  return (
    <>
      <Head>
        <title>{t("termcondition", "Syarat & Ketentuan")}</title>
        <meta
          name="description"
          content={t(
            "termsDescription",
            "Baca syarat dan ketentuan kami untuk informasi lebih lanjut."
          )}
        />
      </Head>
      ...
      <main className="min-h-screen bg-ion-gray py-16 px-6 md:px-12 lg:px-24">
        <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden p-6">
          <h1 className="text-3xl font-bold text-center text-ion-navy mb-6">
            {t("termcondition", "Syarat & Ketentuan")}
          </h1>
          ...
          <div className="mb-6">
            ...
            >
              <span>{t("kodeEtikTitle", "Kode Etik Member ION")}</span>
              <svg
          ...
          {isOpenKodeEtik && (
            <div className="relative w-full h-[80vh] border border-ion-gray rounded-lg overflow-hidden mt-4">
              <iframe
                src="/pdf/kodeetikmemberion.pdf#toolbar=0&navpanes=0&scrollbar=0&view=FitH"
          ...
          <div className="mb-6 mt-8">
            <button
              ...
              style={{ textAlign: "left" }}
            >
              <span>{t("kebijakanTitle", "Kebijakan Kerahasiaan Data")}</span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5"
                fill="none"
          ...
          {isOpenKebijakan && (
            <div className="mt-4 text-gray-800 leading-relaxed space-y-4">
              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.1.title", "Harga Produk")}</h2>
                <p className="ml-4">
                  {t(
                    "kebijakanContent.1.content",
              ...
              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.2.title", "Metode Pembayaran")}</h2>
                <p className="ml-4">{t("kebijakanContent.2.content", "Pembayaran dapat dilakukan melalui metode yang tersedia di platform, termasuk tetapi tidak terbatas pada:")}</p>
                <ul className="list-decimal ml-8 space-y-1">
                  {["ION Cash Wallet", "Transfer bank (virtual account/kliring)", "Kartu kredit/debit", "QRIS atau sistem pembayaran instan lainnya"].map((item, index) => (
                    <li key={index}>{t(`kebijakanContent.2.list.${index}`, item)}</li>
                  ))}
                </ul>
              </div>

              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.3.title", "Kewajiban Pelanggan")}</h2>
                <ul className="list-decimal ml-8 space-y-2">
                  {["Memastikan informasi pembayaran (nomor rekening, kartu) valid dan dimiliki secara sah.", "Mengisi data diri (nama, alamat, kontak) secara lengkap dan akurat.", "Melakukan konfirmasi pembayaran sesuai batas waktu yang ditentukan (misal: upload bukti transfer maksimal 24 jam setelah pemesanan)."].map((item, index) => (
                    <li key={index}>{t(`kebijakanContent.3.list.${index}`, item)}</li>
                  ))}
                </ul>
              </div>

              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.4.title", "Pengelolaan Data")}</h2>
                <p className="ml-4">
                  {t(
                    "kebijakanContent.4.content",
              ...
              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.5.title", "Verifikasi & Konfirmasi Transaksi")}</h2>
                <ul className="list-disc ml-8 space-y-1">
                  {["Untuk transaksi non-tunai, PT Ionaya Optima Natura berhak melakukan verifikasi via SMS/email/telepon guna memastikan keabsahan pembayaran.", "Transaksi dianggap sah setelah pembayaran diterima dan dikonfirmasi oleh sistem atau tim verifikasi kami.", "Untuk pembayaran via transfer bank, pastikan Anda mengirimkan bukti transfer yang jelas dan sesuai nominal."].map((item, index) => (
                    <li key={index}>{t(`kebijakanContent.5.list.${index}`, item)}</li>
                  ))}
                </ul>
              </div>

              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.6.title", "Batas Waktu Pembayaran")}</h2>
                <ul className="list-disc ml-8 space-y-1">
                  {["Pembayaran tidak diterima dalam 24 jam (untuk transfer bank/e-wallet).", "Verifikasi gagal setelah 2x percobaan.", "Terdeteksi kecurangan atau ketidaksesuaian data."].map((item, index) => (
                    <li key={index}>{t(`kebijakanContent.6.list.${index}`, item)}</li>
                  ))}
                </ul>
              </div>

              <div>
                <h2 className="text-xl font-semibold mb-2">{t("kebijakanContent.7.title", "Perubahan Ketentuan")}</h2>
                <p className="ml-4">
                  {t(
                    "kebijakanContent.7.content",
